...
// onloading add the eventlisteners to the buttons, also load the first video
...
// set the effect buttons and effect function
...
// set the buttons and play the selected video
function setVideo(e) {
    var id = e.target.getAttribute("id");
    var video = document.getElementById("video");
    if (id == "video1") {
...
function getFormatExtension() {
    ...
    // canPlayType will return - probably, maybe or empty string
...
function endedHandler(e) {
    // once video has ended reset the play button
...
function processFrame(e) {
    ...
    framenum.innerHTML = "frame " + frameNum;
    frameNum++;
...
function pushUnpushButtons(idToPush, idArrayToUnpush) {
    // the below is to change the class to add the word "selected"
    if (idToPush != "") {
    ...
    // change the class to remove the "selected" if set
...
// check that the button is pushed by looking at the class attributed
// and check that the word "selected" is in the attribute
